...
void GamepadUIButton::FireActionSignal()
{
    ...
    // Замена проблемного шаблона на обычный цикл
    vgui::Panel* pParent = GetParent();
    if (pParent)
    {
        for (int i = 0; i < pParent->GetChildCount(); i++)
        {
            vgui::Panel* pChild = pParent->GetChild(i);
            if (pChild && pChild != this) // Не шлем сообщение самому себе
            {
                // Отправляем стандартное сообщение соседям
                PostMessage(pChild, new KeyValues("OnSiblingGamepadUIButtonOpened"));
            }
        }
    }
}
